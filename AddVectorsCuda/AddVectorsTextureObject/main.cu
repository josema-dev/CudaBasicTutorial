...
__global__ void AddVectors(cudaTextureObject_t a_tex, float *c, const size_t elementNum)
{
	int idx = threadIdx.x + blockDim.x * blockIdx.x;
	if(idx<elementNum)
		c[idx] = tex1Dfetch<float>(a_tex, idx) + tex1Dfetch<float>(texIn, idx);
	//b[idx];
}
...
int main()
{
    ...
    texRes.res.linear.devPtr      = a_dev;
    texRes.res.linear.sizeInBytes = VECTOR_SIZE*sizeof(float);
	...
	AddVectors<<<blocks, threads>>>(triTex, c_dev, VECTOR_SIZE);
